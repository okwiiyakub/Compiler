...
def display_tokens_grouped(tokens):
    ...
    for token in tokens:
        if token.type != "EOF":
            grouped[token.type].append(token.value)
...
def display_tokens_detailed(tokens):
    out = [
        "+----------------------+----------------+------+--------+----------+",
        "| Token Type           | Lexeme         | Line | Column | Position |",
        "+----------------------+----------------+------+--------+----------+",
    ]
    for token in tokens:
        if token.type != "EOF":
            out.append(
                f"| {token.type:<20} | {token.value:<14} | {token.line:<4} | {token.column:<6} | {token.position:<8} |"
            )
    out.append("+----------------------+----------------+------+--------+----------+")
    return "\n".join(out)
