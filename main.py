...
def compile_source(source_code, generate_ast_image=False):
    ...
    try:
        output.append("\n2. SYNTAX ANALYSIS")
        parser=Parser(tokens); compiler_ast=parser.parse()
        output += ["Status: PASSED","Message: Syntax analysis completed successfully.","AST generated successfully."]
        if generate_ast_image:
            try:
                from ast_visualizer import plot_ast
                output.append(f"AST image generated: {plot_ast(compiler_ast)}")
            except Exception as image_error:
                output.append("Warning: AST image was not generated."); output.append(f"Reason: {image_error}")
    except SyntaxErrorCustom as error:
        output += ["Status: FAILED","Error Type: Syntax Error",f"Message: {error}"]
        return "\n".join(output)
    try:
        output.append("\n3. SEMANTIC ANALYSIS")
        analyzer=SemanticAnalyzer(); analyzer.analyze(compiler_ast)
        output += ["Status: PASSED","Message: Semantic analysis completed successfully.","Symbol Table:",analyzer.format_symbol_table()]
